...
from EAS import calculateEAS
from scipy.interpolate import griddata

class EAS_Calculation(QWidget):
    def __init__(self, parent):
        super(EAS_Calculation, self).__init__(parent)
        self.father = parent

        self.setWindowTitle('EASM-R')
    ...
    def on_ok_clicked(self):
        ...
        if position == 0:
            ...
            if self.winsize.text() == '':
                ...
                RR = calculateEAS(dataset, x_spacing, y_spacing, xsize, ysize)
                RR = RR.reshape(-1)

                Z = griddata((x, y), RR, (X, Y), method='cubic')

                mw = MatplotlibWidget()
                mw.mpl.Paint('EAS-相关系数法', X, Y, Z, xunit, yunit, gaunit)
