...
#sigma+小波变换
class Wavelettransform2(QWidget):
    def __init__(self, parent):
        super(Wavelettransform2, self).__init__(parent)
...
# 信号槽
    ...
    def on_ok_clicked(self):
        ...
        if position == 0:
            ...
            sigma_est_Vxx = estimate_sigma(Vxx, multichannel=True, average_sigmas=True)
            sigma_est_Vxy = estimate_sigma(Vxy, multichannel=True, average_sigmas=True)
            sigma_est_Vxz = estimate_sigma(Vxz, multichannel=True, average_sigmas=True)
            sigma_est_Vyy = estimate_sigma(Vyy, multichannel=True, average_sigmas=True)
            sigma_est_Vyz = estimate_sigma(Vyz, multichannel=True, average_sigmas=True)
            sigma_est_Vzz = estimate_sigma(Vzz, multichannel=True, average_sigmas=True)
            sigma_est_Vz = estimate_sigma(Vz, multichannel=True, average_sigmas=True)

            im_haar_Vxx = denoise_wavelet(Vxx, mode=mode2, wavelet_levels=int(level2), wavelet=str(dbx), multichannel=True, sigma=sigma_est_Vxx)
            im_haar_Vxy = denoise_wavelet(Vxy, mode=mode2, wavelet_levels=int(level2), wavelet=str(dbx), multichannel=True, sigma=sigma_est_Vxy)
            im_haar_Vxz = denoise_wavelet(Vxz, mode=mode2, wavelet_levels=int(level2), wavelet=str(dbx), multichannel=True, sigma=sigma_est_Vxz)
            im_haar_Vyy = denoise_wavelet(Vyy, mode=mode2, wavelet_levels=int(level2), wavelet=str(dbx), multichannel=True, sigma=sigma_est_Vyy)
            im_haar_Vyz = denoise_wavelet(Vyz, mode=mode2, wavelet_levels=int(level2), wavelet=str(dbx), multichannel=True, sigma=sigma_est_Vyz)
            im_haar_Vzz = denoise_wavelet(Vzz, mode=mode2, wavelet_levels=int(level2), wavelet=str(dbx), multichannel=True, sigma=sigma_est_Vzz)
            im_haar_Vz = denoise_wavelet(Vz, mode=mode2, wavelet_levels=int(level2), wavelet=str(dbx), multichannel=True, sigma=sigma_est_Vz)
