...
from THDR import calculateTHDR
from scipy.interpolate import griddata

class THDR_Calculation(QWidget):
    def __init__(self, parent):
        super(THDR_Calculation, self).__init__(parent)
        self.father = parent

        self.setWindowTitle('总水平导数法')
    ...
    def on_ok_clicked(self):
        ...
        Thdr = calculateTHDR(dataset, x_spacing, y_spacing)
        Thdr = Thdr.reshape(-1)

        Z = griddata((x, y), Thdr, (X, Y), method='cubic')
        mw = MatplotlibWidget()
        mw.mpl.Paint('总水平导数法', X, Y, Z, xunit, yunit, gaunit)

        np.savetxt("./record/Edgeresult.txt", Thdr)
