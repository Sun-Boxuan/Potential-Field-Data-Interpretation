...
from ED_A import calculateEDA
from scipy.interpolate import griddata

class ED_ACalculation(QWidget):
    def __init__(self, parent):
        super(ED_ACalculation, self).__init__(parent)
        self.father = parent

        self.setWindowTitle('水平导数法')
    ...
    def on_ok_clicked(self):
        ...
        EDA = calculateEDA(dataset, x_spacing, y_spacing)
        EDA = EDA.reshape(-1)

        Z = griddata((x, y), EDA, (X, Y), method='cubic')

        mw = MatplotlibWidget()
        mw.mpl.Paint('总水平导数法', X, Y, Z, xunit, yunit, gaunit)

        np.savetxt("./record/Edgeresult.txt", EDA)
