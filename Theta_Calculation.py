...
from Theta import calculateTheta
from scipy.interpolate import griddata

class Theta_Calculation(QWidget):
    def __init__(self, parent):
        super(Theta_Calculation, self).__init__(parent)
        self.father = parent

        self.setWindowTitle('Theta图法')
    ...
    def on_ok_clicked(self):
        ...
        Theta = calculateTheta(dataset, x_spacing, y_spacing)
        Theta = Theta.reshape(-1)

        Z = griddata((x, y), Theta, (X, Y), method='cubic')
        mw = MatplotlibWidget()
        mw.mpl.Paint('Theta图法', X, Y, Z, xunit, yunit, gaunit)

        np.savetxt("./record/Edgeresult.txt", Theta)
