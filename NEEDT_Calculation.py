...
from NEEDT import calculateNEEDT
from scipy.interpolate import griddata

class NEEDT_Calculation(QWidget):
    def __init__(self, parent):
        super(NEEDT_Calculation, self).__init__(parent)
        self.father = parent

        self.setWindowTitle('归一化的改进方向总水平导数法')
        ...
        self.label5 = QLabel('Parameter P:')
        self.p0 = QLineEdit()
        self.p0.setText('0.1')

        layoutline3.addWidget(self.label5)
        layoutline3.addWidget(self.p0)
    ...
    def on_ok_clicked(self):
        ...
        NEEDT = calculateNEEDT(dataset, x_spacing, y_spacing, calp)
        NEEDT = NEEDT.reshape(-1)

        Z = griddata((x, y), NEEDT, (X, Y), method='cubic')
        mw = MatplotlibWidget()
        mw.mpl.Paint('归一化的改进方向总水平导数法', X, Y, Z, xunit, yunit, gaunit)


        np.savetxt("./record/Edgeresult.txt", NEEDT)
        ...
        sub = QMdiSubWindow()
        sub.setWidget(mw)
        sub.setWindowTitle('归一化的改进方向总水平导数法')
        ...
        child.setText(0, '归一化的改进方向总水平导数法')
        # 设置树上root为选中状态
