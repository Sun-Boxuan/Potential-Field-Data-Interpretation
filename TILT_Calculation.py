...
class TILT_Calculation(QWidget):
    def __init__(self, parent):
        super(TILT_Calculation, self).__init__(parent)
        self.father = parent

        self.setWindowTitle('倾斜角法')
    ...
    def on_ok_clicked(self):
        ...
        TILT = calculateTILT(dataset, x_spacing, y_spacing)
        TILT = TILT.reshape(-1)

        Z = griddata((x, y), TILT, (X, Y), method='cubic')
        mw = MatplotlibWidget()
        mw.mpl.Paint('倾斜角法', X, Y, Z, xunit, yunit, gaunit)

        np.savetxt("./record/Edgeresult.txt", TILT)
