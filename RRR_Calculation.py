...
from RRR import calculateRRR
from scipy.interpolate import griddata

class RRR_Calculation(QWidget):
    def __init__(self, parent):
        super(RRR_Calculation, self).__init__(parent)
        self.father = parent

        self.setWindowTitle('相关系数法')
...
# 界面
        ...
        self.label5 = QLabel('计算窗口的尺寸:')
        self.winsize = QLineEdit()
        self.winsize.setText('3')
    ...
    def on_ok_clicked(self):
        ...
        if position == 0:
            ...
            if self.winsize.text() == '':
                ...
                RR = calculateRRR(dataset, x_spacing, y_spacing, calp, xsize, ysize)
                RR = RR.reshape(-1)

                Z = griddata((x, y), RR, (X, Y), method='cubic')

                mw = MatplotlibWidget()
                mw.mpl.Paint('相关系数法', X, Y, Z, xunit, yunit, gaunit)

                np.savetxt("./record/Edgeresult.txt", RR)
