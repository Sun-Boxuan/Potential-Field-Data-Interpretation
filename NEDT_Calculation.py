...
class NEDT_Calculation(QWidget):
    def __init__(self, parent):
        super(NEDT_Calculation, self).__init__(parent)
        self.father = parent

        self.setWindowTitle('归一化方向总水平导数法')
    ...
    def on_ok_clicked(self):
        ...
        NEDT = calculateNEDT(dataset, x_spacing, y_spacing, calp)
        NEDT = NEDT.reshape(-1)

        Z = griddata((x, y), NEDT, (X, Y), method='cubic')
        mw = MatplotlibWidget()
        mw.mpl.Paint('归一化方向总水平导数法', X, Y, Z, xunit, yunit, gaunit)

        np.savetxt("./record/Edgeresult.txt", NEDT)
        ...
        sub = QMdiSubWindow()
        sub.setWidget(mw)
        sub.setWindowTitle('归一化方向总水平导数法')
