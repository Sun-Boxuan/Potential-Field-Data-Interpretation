...
from EDT import calculateEDT
from scipy.interpolate import griddata

class EDT_Calculation(QWidget):
    def __init__(self, parent):
        super(EDT_Calculation, self).__init__(parent)
        self.father = parent

        self.setWindowTitle('方向总水平导数法')
    ...
    def on_ok_clicked(self):
        ...
        EDT = calculateEDT(dataset, x_spacing, y_spacing)
        EDT = EDT.reshape(-1)

        Z = griddata((x, y), EDT, (X, Y), method='cubic')

        mw = MatplotlibWidget()
        mw.mpl.Paint('方向总水平导数法', X, Y, Z, xunit, yunit, gaunit)

        np.savetxt("./record/Edgeresult.txt", EDT)
