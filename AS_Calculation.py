...
from AS import calculateAS
from MatplotlibWidget import MatplotlibWidget
from scipy.interpolate import griddata

class AS_Calculation(QWidget):
    def __init__(self, parent):
        super(AS_Calculation, self).__init__(parent)
        self.father = parent

        self.setWindowTitle('解析信号法')
...
# 信号槽
    ...
    def on_ok_clicked(self):
        ...
        AS = calculateAS(dataset, x_spacing, y_spacing)
        AS = AS.reshape(-1)

        Z = griddata((x, y), AS, (X, Y), method='cubic')

        mw = MatplotlibWidget()
        mw.mpl.Paint('解析信号法', X, Y, Z, xunit, yunit, gaunit)

        np.savetxt("./record/Edgeresult.txt", AS)
